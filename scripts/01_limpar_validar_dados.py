...
def limpar_dados(df):
    ...
    # Remove espaços extras nos nomes das colunas (boa prática sempre)
    ...
    # Dropna limpa os registros nulos (essa base de água costuma ter muitos nulos em 'ph' e 'Sulfate')
    df = df.dropna()

    # Garante que a potabilidade seja um número inteiro binário (0 ou 1)
...
def validar_com_great_expectations(df):
    ...
    expectativas = [
        ...
        # Garante que colunas críticas não possuem nulos após a limpeza
        gx.expectations.ExpectColumnValuesToNotBeNull(column="ph"),
        gx.expectations.ExpectColumnValuesToNotBeNull(column="Sulfate")
